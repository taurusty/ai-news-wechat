...
class VentureBeatSource(BaseSource):
    """VentureBeat - 抓取（偏AI相关，通过后续筛选/分类处理）"""

    def __init__(self, weight: float = 1.0, enabled: bool = True):
        super().__init__(base_url="https://venturebeat.com/", source_name="venturebeat", weight=weight, enabled=enabled)

    async def fetch_articles(self, max_items: int = 10) -> List[Article]:
        list_url = "https://venturebeat.com/"  # 后续可改成其AI/频道页
        html = await self.fetch(list_url, headers={"User-Agent": "Mozilla/5.0"})
        soup = BeautifulSoup(html, "lxml")

        links = []
        for a in soup.select('a[href^="https://venturebeat.com/"]'):
            href = a.get('href')
            if not href:
                continue
            # 排除分类页
            ...
            if len(title) < 8:
                continue
            if href not in [u for u, _ in links]:
